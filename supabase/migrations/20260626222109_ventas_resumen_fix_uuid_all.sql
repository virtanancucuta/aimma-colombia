-- AIMMA · Ventas · Fase 2 · Fix intermedio (SUPERSEDED por 20260626222325). Castea los 3 uuid en max().
-- Quedo incompleto: al cambiar la group-key a producto_id, 'referencia' quedo sin agregar -> error
-- "l.ref_act must appear in GROUP BY". Se conserva por lockstep. La version viva es 20260626222325.
...
begin
  ...
  agg as (
    select coalesce(l.ref_act, l.ref_snap) as referencia,
