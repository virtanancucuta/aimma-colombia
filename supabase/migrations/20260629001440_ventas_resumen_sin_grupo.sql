-- AIMMA · Modulo Ventas · Fase 5 Mejora 3a: drill de "Sin proveedor" / "Sin categoria".
-- ventas_resumen + p_sin_proveedor / p_sin_categoria (booleanos default false). Agregar PARAMETROS cambia la
-- FIRMA (overload) -> DROP+CREATE (no CREATE OR REPLACE puro), igual que p_telefono. Cuerpo VERBATIM del actual
-- + 2 lineas nuevas en el WHERE. Cuando los booleanos van false (default), no filtran -> callers existentes
-- intactos. Cuando true -> exigen proveedor_id/categoria_id IS NULL (drill del grupo "sin grupo").
drop function public.ventas_resumen(uuid, date, date, text, uuid, uuid, text, integer, integer, text);

create function public.ventas_resumen(p_tienda_id uuid, p_desde date DEFAULT NULL::date, p_hasta date DEFAULT NULL::date, p_orden text DEFAULT NULL::text, p_proveedor_id uuid DEFAULT NULL::uuid, p_categoria_id uuid DEFAULT NULL::uuid, p_buscar text DEFAULT NULL::text, p_limit integer DEFAULT NULL::integer, p_offset integer DEFAULT 0, p_telefono text DEFAULT NULL::text, p_sin_proveedor boolean DEFAULT false, p_sin_categoria boolean DEFAULT false)
...
begin
  ...
  with venta_items as (
    ...
    where p.tienda_id = p_tienda_id and p.estado in ('cerrado','devuelto')
      ...
      and (not p_sin_proveedor or pr.proveedor_id is null)   -- << NUEVO: drill "Sin proveedor"
      and (not p_sin_categoria or pr.categoria_id is null)   -- << NUEVO: drill "Sin categoria"
  ),
  lineas as (
...
$function$;
revoke all on function public.ventas_resumen(uuid, date, date, text, uuid, uuid, text, integer, integer, text, boolean, boolean) from public, anon;
grant execute on function public.ventas_resumen(uuid, date, date, text, uuid, uuid, text, integer, integer, text, boolean, boolean) to authenticated;
