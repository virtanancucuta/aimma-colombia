-- AIMMA · Modulo Ventas · Fase 2 · Fix ventas_resumen (deja la funcion ejecutable + group-by C correcto).
-- Supersede a 215800 (resumen original) y a los fixes parciales 221837 / 222109. Esta es la version viva.
-- Cambios vs 215800: (1) max() sobre uuid no existe -> cast a text: producto_id/proveedor_id/categoria_id
-- como max(x::text)::uuid; (2) group-by C: coalesce(producto_id::text,'snap:'||ref_snap) (robusto ante
-- hard-delete+reuso de referencia, FK producto_id ON DELETE SET NULL); (3) referencia agregada:
-- coalesce(max(ref_act),max(ref_snap)) (ya no es la group-key). CREATE OR REPLACE preserva grants.
