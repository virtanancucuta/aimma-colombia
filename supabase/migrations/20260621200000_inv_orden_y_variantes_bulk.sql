-- ============================================================================
-- INVENTARIO GENERAL — orden por cantidad/valor (asc/desc) + inventario_variantes
-- a ARRAY de producto_ids (sirve drill-down con 1 id y export con N ids).
--   inventario_resumen: +3 casos de orden (valor_asc, cantidad_desc, cantidad_asc).
--   inventario_variantes: firma (uuid, uuid[], int), devuelve producto_id para agrupar.
-- Mismo candado/seguridad. #variable_conflict use_column.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- inventario_resumen (CREATE OR REPLACE) — identico a 20260621170100 + 3 ordenes.
-- ----------------------------------------------------------------------------
...
begin
  ...
  stk as (
    ...
    where pv.producto_id in (select id from base) group by pv.producto_id
  ),
  vta as (
    select im.producto_id,
           (-1 * coalesce(sum(im.cantidad) filter (
              where im.tipo in ('venta','devolucion') and im.created_at >= now() - make_interval(days => v_periodo)),0))::bigint as unidades_vendidas,
    ...
    where im.producto_id in (select id from base) group by im.producto_id
  ),
  metrics as (
    select b.id, b.referencia, b.nombre, b.foto_principal_url,
           ...
           v.fecha_ultima_venta, coalesce(v.fecha_primera_entrada, b.created_at) as fecha_ingreso
  ...
  computed as (
    select m.*,
           ...
           case when m.stock_total = 0 then 0::numeric
                ...
                else m.stock_total::numeric / (m.unidades_vendidas::numeric / m.dias_efectivos) end as dias_inventario,
  ...
  clasif as (
    select c.*,
      case when c.stock_total = 0 then 'quiebre'
  ...
  filtered as (
    select * from clasif where (p_clasificacion is null or clasif.clasificacion = any(p_clasificacion))
  )
  select f.id, f.referencia, f.nombre, f.foto_principal_url,
    f.proveedor_id, f.proveedor_nombre, f.categoria_id, f.categoria_nombre,
    f.stock_total, f.reservado_total, (f.stock_total - f.reservado_total)::bigint,
    f.costo_unitario, f.valor_inventario, f.unidades_vendidas, f.dias_efectivos, f.venta_diaria, f.dias_inventario,
    f.sin_ventas, f.datos_insuficientes, f.fecha_ultima_venta, f.fecha_ingreso, f.clasificacion,
    count(*) over()::bigint
  from filtered f
  order by
...
-- ----------------------------------------------------------------------------
-- inventario_variantes -> array de producto_ids (drill + export). Devuelve producto_id.
-- ----------------------------------------------------------------------------
