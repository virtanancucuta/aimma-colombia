-- AIMMA · Ventas · Fase 2 · Fix intermedio (SUPERSEDED por 20260626222325). Solo casteo producto_id.
-- Quedo incompleto: proveedor_id/categoria_id (tambien uuid) seguian con max() crudo -> aun erraba en runtime.
-- Se conserva por lockstep (esta version fue aplicada). La version viva es 20260626222325.
...
begin
  ...
  agg as (
    select coalesce(l.ref_act, l.ref_snap) as referencia, max(l.producto_id::text)::uuid as producto_id,
