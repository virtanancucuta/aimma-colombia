...
- **Admin → iframe** (nuevo en `editor-canvas.js`): `applyThemePreview(vars, fontUrl)` → `iframe.contentWindow.postMessage({ type:'theme', vars, fontUrl }, tenantOrigin)`. `vars` = mapa `{'--ta-color-primary':'#..', ..., '--ta-color-on-primary':'#..' (auto-contrast calculado en el admin), '--ta-font-display':'..', '--ta-font-body':'..'}`.
- **Storefront preview bridge** (nuevo handler en `index.astro`, SOLO `isPreview`): `if (msg.type === 'theme') { for (k,v of msg.vars) document.documentElement.style.setProperty(k, v); if (msg.fontUrl) ensureFontLink(msg.fontUrl); }`. `ensureFontLink` agrega un `<link rel=stylesheet href=fontUrl>` si no existe (idempotente por href). Aplica instantáneo (el `<html style>` ya define los `--ta-*`; setProperty los pisa).
- El cálculo de los `--ta-color-on-*` (auto-contrast WCAG) se replica en el admin (port de `getContrastText`, o un helper compartido mínimo) para que el preview sea fiel.
