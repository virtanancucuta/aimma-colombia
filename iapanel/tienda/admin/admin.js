...
(function () {
  ...
  const ROUTES = ['', 'productos', 'inventario', 'ventas', 'categorias', 'crm', 'pedidos', 'resenas', 'configuracion', 'legales', 'vista-previa', 'editor', 'fotos-ia'];
  const DEFAULT_ROUTE = '';
  ...
  async function loadProfileAndTienda(userId) {
    ...
    const { data: tienda, error: tErr } = await supabase
      .from('tiendas')
      .select('id, user_id, slug, nombre_negocio, logo_url, plantilla_id, paleta_id, estado, idioma, whatsapp_dueno, mostrar_agotados, mostrar_buscador_header, hover_segunda_foto, mostrar_resenas_productos, factura_con_iva, tasa_iva_default, inv_umbral_ruptura_dias, inv_umbral_optimo_dias, inv_umbral_sobrestock_dias, inv_periodo_default_dias, nombre_legal, nit, direccion, ciudad_negocio, email_contacto, telefono_contacto, sync_dashboard_excel_activo, cortesia_razon, plan_tienda, horario_atencion, easypanel_domain_id, subdominio_publicado_at, created_at, updated_at, personalizaciones')
      .eq('user_id', userId)
      .maybeSingle();
    if (tErr) throw tErr;
